    ...
    for index in tqdm(predictions.keys(), ncols=80):
        ...
        for i in range(0, len(prediction), window_frame_size):
            ...
            if len(pred_window) < 9:
                print(
                    f"Window frame size of {len(pred_window)} "
                    f"is smaller than minimum pad length of 9. "
                    f"Window ignored!"
                )
                continue
            ...
            if (
                ...
                or config.TEST.DATA.PREPROCESS.LABEL_TYPE == "Raw"
            ):
    ...
    if config.INFERENCE.EVALUATION_METHOD == "FFT":
        ...
        for metric in config.TEST.METRICS:
            ...
            if metric == "MAE":
                ...
                MAE_FFT = np.mean(
                    np.abs(predict_hr_fft_all - gt_hr_fft_all)
                )
                ...
                squared_errors = np.square(
                    predict_hr_fft_all - gt_hr_fft_all
                )
                ...
                print(
                    "FFT Pearson (FFT Label): {0} +/- {1}".format(
                ...
                print(
                    "FFT SNR (FFT Label): {0} +/- {1} (dB)".format(
                        SNR_FFT,
                        standard_error
                    )
                )
        ...
        for metric in config.TEST.METRICS:
            ...
            if metric == "MAE":
                ...
                MAE_PEAK = np.mean(
                    np.abs(predict_hr_peak_all - gt_hr_peak_all)
                )
                ...
                print(
                    "Peak MAE (Peak Label): {0} +/- {1}".format(
                        MAE_PEAK,
                        standard_error
                    )
                )
                ...
                squared_errors = np.square(
                    predict_hr_peak_all - gt_hr_peak_all
                )
                ...
                print(
                    "PEAK RMSE (Peak Label): {0} +/- {1}".format(
                        RMSE_PEAK,
                        standard_error
                    )
                )
