...
def _compute_macc(pred_signal, gt_signal):
    """Calculate maximum amplitude of cross correlation (MACC)."""
...
def _calculate_SNR(
    ...
    if not signal_power_rem == 0:
        SNR = power2db(
            (signal_power_hm1 + signal_power_hm2)
            / signal_power_rem
        )
    else:
        SNR = 0
...
def calculate_metric_per_video(
    ...
    if use_bandpass:
        ...
        predictions = scipy.signal.filtfilt(
            b,
            a,
            np.double(predictions)
        )

        labels = scipy.signal.filtfilt(
            b,
            a,
            np.double(labels)
        )
    ...
    if hr_method == 'FFT':
        ...
        hr_pred = _calculate_peak_hr(
            predictions,
            fs=fs
        )

        hr_label = _calculate_peak_hr(
            labels,
            fs=fs
        )
        ...
        raise ValueError(
            'Please use FFT or Peak to calculate your HR.'
        )
